...
const inp =
  "w-full bg-[#0e0e0e] border border-[#333] rounded-lg px-3 py-2 text-sm text-white placeholder-[#555] focus:outline-none focus:border-[#b794f4] transition";
const chip = (active: boolean) =>
  `px-3 py-1.5 rounded-lg text-sm cursor-pointer border transition select-none ${
    active
      ? "border-[#b794f4] text-[#b794f4] bg-[#1a0a2e]"
      : "border-[#2a2a2a] text-[#666] bg-[#13131a] hover:border-[#444]"
  }`;
...
function MyShop() {
  ...
  const [consultProducts, setConsultProducts] = useState<ConsultProduct[]>(() =>
    loadStored<ConsultProduct[]>("consultProducts", [
      {
        id: 1,
        type: "tarot",
        name: "타로/사주 10분 상담",
  ...
  const [saleProducts, setSaleProducts] = useState<SaleProduct[]>(() =>
    loadStored<SaleProduct[]>("saleProducts", [
      {
        id: 1,
        kind: "ebook",
        title: "",
  ...
  const [pdfProducts, setPdfProducts] = useState<PdfProduct[]>(() =>
    loadStored<PdfProduct[]>("pdfProducts", [
      {
        id: 1,
        type: PDF_TYPES[0],
  ...
  const addConsult = () =>
    setConsultProducts((p) => [
      ...p,
      { id: Date.now(), type: "tarot", name: "", time: "10분", price: "", desc: "", badge: "" },
    ]);
  ...
  const toggleSale = (id: number) =>
    setSaleProducts((p) => p.map((x) => (x.id === id ? { ...x, open: !x.open } : x)));
      ...
      <div className="max-w-[600px] mx-auto px-4 pt-6">
            ...
            <button
              ...
              className={`px-3 py-2 rounded-lg text-xs border transition whitespace-nowrap ${
                copied
                  ...
                  : "border-[#b794f4] text-[#b794f4] hover:bg-[#1a0a2e]"
              }`}
            >
              {copied ? "✓ 복사됨" : "복사"}
            </button>
        ...
        <div className="bg-[#13131a] border border-[#2a2a2a] rounded-xl p-5 mb-4">
          ...
          <div className="grid grid-cols-4 gap-2 mb-4">
            {FONTS.map((f) => (
              <button key={f} onClick={() => setFont(f)} className={chip(font === f)}>
                {f}
              </button>
            ))}
          </div>
          ...
          <div className="flex gap-2 mb-4">
            {FONT_SIZES.map((s) => (
              <button key={s} onClick={() => setFontSize(s)} className={chip(fontSize === s)}>
                {s}
              </button>
            ))}
          </div>
        ...
        <div className="bg-[#13131a] border border-[#2a2a2a] rounded-xl p-5 mb-4">
          ...
          <div className="flex gap-2 mb-1">
            {FREE_TIMES.map((t) => (
              <button key={t} onClick={() => setFreeTime(t)} className={chip(freeTime === t)}>
                {t}
              </button>
            ))}
          </div>
        ...
        <div className="bg-[#13131a] border border-[#2a2a2a] rounded-xl p-5 mb-4">
          <div className="flex items-center justify-between mb-4">
            ...
            <button
              onClick={addConsult}
              className="text-xs border border-[#b794f4] text-[#b794f4] px-3 py-1 rounded-lg hover:bg-[#1a0a2e] transition"
            >
              + 추가
            </button>
          </div>
            ...
            {consultProducts.map((p, i) => (
              <div key={p.id} className="bg-[#0e0e0e] border border-[#222] rounded-xl p-4">
                ...
                <div className="flex gap-3 mb-3">
                  <img
                    src={CONSULT_IMG[p.type] || CONSULT_IMG.tarot}
                    alt={p.type}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <select
                      className={inp}
                      value={p.type}
                      onChange={(e) => updateConsult(p.id, "type", e.target.value)}
                    >
                      {CONSULT_TYPES.map((t) => (
                ...
                <input
                  className={inp + " mb-3"}
                  value={p.name}
                  onChange={(e) => updateConsult(p.id, "name", e.target.value)}
                  placeholder="상품 이름 (예: 기본 타로 상담)"
                />
                <div className="grid grid-cols-3 gap-3 mb-3">
                  <div>
                    ...
                    <select
                      className={inp}
                      value={p.time}
                      onChange={(e) => updateConsult(p.id, "time", e.target.value)}
                    >
                      {TIMES.map((t) => (
                  ...
                  <div>
                    ...
                    <input
                      ...
                      type="number"
                      value={p.price}
                      onChange={(e) => updateConsult(p.id, "price", e.target.value)}
                    />
                  </div>
                  <div>
                    ...
                    <select
                      className={inp}
                      value={p.badge}
                      onChange={(e) => updateConsult(p.id, "badge", e.target.value)}
                    >
                ...
                <button
                  onClick={() =>
                    payWithToss({
                      name:
                        (p.name || CONSULT_TYPES.find((t) => t.value === p.type)?.label || "상담") +
                        " " +
                        p.time,
        ...
        <div className="bg-[#13131a] border border-[#2a2a2a] rounded-xl p-5 mb-4">
          ...
          <div className="flex gap-3 mb-4">
            <button
              onClick={() => addSale("ebook")}
              className="flex-1 border border-[#2a2a2a] text-[#888] text-sm py-2 rounded-lg hover:border-[#b794f4] hover:text-[#b794f4] transition"
            >
              📖 전자책 추가
            </button>
            <button
              onClick={() => addSale("physical")}
              className="flex-1 border border-[#2a2a2a] text-[#888] text-sm py-2 rounded-lg hover:border-[#b794f4] hover:text-[#b794f4] transition"
            >
              🎁 실물상품 추가
            </button>
          ...
          <div className="space-y-2">
            {saleProducts.map((p) => (
              <div
                key={p.id}
                className="bg-[#0e0e0e] border border-[#222] rounded-xl overflow-hidden"
              >
                <button
                  onClick={() => toggleSale(p.id)}
                  className="w-full flex items-center justify-between px-4 py-3 hover:bg-[#111] transition"
                >
                  ...
                  <div className="px-4 pb-4 space-y-3 border-t border-[#222]">
                    <div className="mt-3">
                      ...
                      <input
                        className={inp}
                        value={p.title}
                        onChange={(e) => updateSale(p.id, "title", e.target.value)}
                        placeholder={
                          p.kind === "ebook"
                            ? "예: 타로 입문 전자책"
                            : "예: 프리미엄 타로 카드 세트"
                    ...
                    <div>
                      ...
                      <textarea
                        ...
                        placeholder={
                          p.kind === "ebook"
                            ? "전자책에 대한 설명을 입력하세요"
                            : "실물상품에 대한 설명을 입력하세요 (구성품, 재질, 사이즈 등)"
                        }
                      />
                    </div>
                    <div>
                    ...
                    <div className="grid grid-cols-3 gap-3">
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => updateSale(p.id, "regularPrice", e.target.value)}
                        />
                      </div>
                      <div>
                        ...
                        <input
                          ...
                          type="number"
                          value={p.salePrice}
                          onChange={(e) => updateSale(p.id, "salePrice", e.target.value)}
                        />
                      </div>
                      <div>
                        <label className="text-xs text-[#555] mb-1 block">할인율</label>
                        <div className={inp + " text-center text-[#f6ad55]"}>
                          {discount(p.regularPrice, p.salePrice)}
                        </div>
                      </div>
                    </div>
                    {p.kind === "ebook" && (
                      <div>
                        ...
                        <input
                          ...
                          type="number"
                          value={p.pages}
                          onChange={(e) => updateSale(p.id, "pages", e.target.value)}
                        />
                      </div>
                    )}
                    <div>
                      ...
                      <input
                        className={inp}
                        value={p.buyLink}
                        onChange={(e) => updateSale(p.id, "buyLink", e.target.value)}
                        placeholder="https://litt.ly/..."
                      />
                      <p className="text-xs text-[#555] mt-1">
                        고객이 '구매하기' 클릭 시 이 링크로 이동합니다
                      </p>
                    </div>
                    <button
                      onClick={() => removeSale(p.id)}
        ...
        <div className="bg-[#13131a] border border-[#2a2a2a] rounded-xl p-5 mb-4">
          ...
          <p className="text-xs text-[#555] mb-4">
            상담 종료 시 고객에게 자동 제안되는 AI 사주/궁합 분석 PDF 상품입니다
          </p>
          <div className="space-y-4">
            {pdfProducts.map((p, i) => (
              <div key={p.id} className="bg-[#0e0e0e] border border-[#222] rounded-xl p-4">
                ...
                <div className="flex gap-3 mb-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <select
                      className={inp}
                      value={p.type}
                      onChange={(e) => updatePdf(p.id, "type", e.target.value)}
                    >
                      {PDF_TYPES.map((t) => (
                        <option key={t}>{t}</option>
                      ))}
                ...
                <input
                  className={inp + " mb-3"}
                  value={p.name}
                  onChange={(e) => updatePdf(p.id, "name", e.target.value)}
                  placeholder="상품 이름"
                />
                <textarea
                  className={inp + " mb-3 resize-none"}
                  rows={3}
                  value={p.desc}
                  onChange={(e) => updatePdf(p.id, "desc", e.target.value)}
                  placeholder="고객에게 보여질 상품 소개"
                />
                <div className="grid grid-cols-2 gap-3 mb-3">
                  <div>
                    ...
                    <input
                      ...
                      type="number"
                      value={p.price}
                      onChange={(e) => updatePdf(p.id, "price", e.target.value)}
                    />
                  </div>
                  <div>
                    <label className="text-xs text-[#555] mb-1 block">뱃지</label>
                    <select
                      className={inp}
                      value={p.badge}
                      onChange={(e) => updatePdf(p.id, "badge", e.target.value as string)}
                    >
                      <option value="">없음</option>
                      <option value="인기">인기</option>
                      <option value="추천">추천</option>
                ...
                <div className="flex items-center gap-3">
                  <button
                    onClick={() => updatePdf(p.id, "active", !p.active)}
                    className={`w-10 h-5 rounded-full transition-colors relative ${p.active ? "bg-[#b794f4]" : "bg-[#333]"}`}
                  >
                    <span
                      className={`absolute top-0.5 w-4 h-4 bg-white rounded-full shadow transition-transform ${p.active ? "translate-x-5" : "translate-x-0.5"}`}
                    />
                  </button>
